...
const handleSendInfo = () => {
    alert("La fonctionnalité d'envoi (P2P) sera bientôt disponible !");
};
</script>
...
<template>
      ...
      <div class="flex-1 w-full relative">
        ...
        <div class="md:hidden bg-gray-50 min-h-screen pb-0 -mx-4 -mt-2">
            ...
            <div class="px-6 -mt-4 relative z-20">
                <div class="bg-white p-1.5 rounded-2xl shadow-lg flex gap-1 border border-gray-100 dark:border-gray-700">
                    <button 
        ...
        <div class="hidden md:block">
            <div class="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p class="text-gray-500 dark:text-gray-400">Gérez vos revenus et l'historique de vos transactions</p>
                </div>
                <div class="flex gap-3">
                    <button 
                        @click="handleSendInfo"
                        class="px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-xl text-sm font-bold text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors flex items-center gap-2"
                    >
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <!-- Pending -->
                <div class="bg-transparent md:bg-white dark:bg-gray-800 p-6 rounded-xl shadow-none md:shadow border-l-4 border-yellow-500">
                    <div class="flex items-center justify-between mb-2">
                        <h3 class="text-gray-500 dark:text-gray-400 text-sm font-medium uppercase tracking-wider">En Attente</h3>
                        <i class="fas fa-clock text-yellow-500"></i>
                    </div>
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ formatPrice(summary.pendingValue) }}</p>
                    <p class="text-gray-400 text-xs mt-2">Transferts en cours</p>
                </div>

                <!-- Total Received -->
                <div class="bg-transparent md:bg-white dark:bg-gray-800 p-6 rounded-xl shadow-none md:shadow border-l-4 border-green-500">
                    <div class="flex items-center justify-between mb-2">
                        <h3 class="text-gray-500 dark:text-gray-400 text-sm font-medium uppercase tracking-wider">Total Reçu</h3>
                        <i class="fas fa-check-circle text-green-500"></i>
                    </div>
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ formatPrice(summary.totalPaid) }}</p>
                    <p class="text-gray-400 text-xs mt-2">Depuis le début</p>
                </div>
            </div>

            <!-- DESKTOP TABS -->
            <div class="flex gap-4 mb-6 border-b border-gray-100 dark:border-gray-700 mt-8">
                <button 
                    @click="currentTab = 'sales'"
                    :class="[
            ...
            <!-- SALES TABLE (Desktop) -->
            <div v-if="currentTab === 'sales'" class="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-100 dark:border-gray-700 overflow-hidden">
                ...
                <div v-else-if="transactions.length === 0" class="p-12 text-center">
                    ...
                    <h4 class="text-lg font-medium text-gray-900 dark:text-white">Aucune vente</h4>
                            ...
                            <tr v-for="tx in transactions" :key="tx.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        ...
                                        <div>
                                            <p class="text-sm font-bold text-gray-900 dark:text-white truncate max-w-[150px]">{{ tx.productName }}</p>
                ...
                <!-- Sales Pagination -->
                <div v-if="!transactionsLoading && transactionsPagination.totalPages > 1" class="px-6 py-4 bg-gray-50 dark:bg-gray-900 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
                    <span class="text-xs text-gray-500">Page {{ transactionsPagination.page }} sur {{ transactionsPagination.totalPages }}</span>
                    <div class="flex space-x-2">
                        <button 
                            @click="changePage(transactionsPagination.page - 1)" 
                            :disabled="transactionsPagination.page === 1"
                            class="p-1 px-3 border border-gray-200 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 text-xs disabled:opacity-50 hover:bg-gray-50"
                        >
            ...
            <div v-if="currentTab === 'payouts'" class="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-100 dark:border-gray-700 overflow-hidden">
                <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-700 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    <h3 class="text-lg font-bold text-gray-900 dark:text-white">Historique des retraits</h3>
                    <div class="flex items-center gap-3">
                        ...
                        <input 
                            ...
                            class="block w-full pl-9 pr-3 py-1.5 border border-gray-200 dark:border-gray-100 rounded-lg text-xs bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-1 focus:ring-blue-500"
                        />
                        </div>
                    </div>
                </div>
                ...
                <div v-else-if="payouts.length === 0" class="p-12 text-center">
                    ...
                    <h4 class="text-lg font-medium text-gray-900 dark:text-white">Aucun retrait</h4>
                ...
                <div v-else class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700">
                            <tr v-for="payout in payouts" :key="payout.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                ...
                <!-- Payouts Pagination -->
                <div v-if="!loading && pagination && pagination.totalPages > 1" class="px-6 py-4 bg-gray-50 dark:bg-gray-900 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
  ...
  <div v-if="showWithdrawModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-3xl w-full max-w-md relative z-10 shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        <div class="p-6">
            <div class="flex justify-between items-center mb-6">
                <h3 class="text-xl font-bold text-gray-900 dark:text-white">Demander un Retrait</h3>
                <button @click="showWithdrawModal = false" class="text-gray-400 hover:text-gray-600">
                    <i class="fas fa-times"></i>
                </button>
            </div>

            <div class="mb-6 bg-blue-50 dark:bg-blue-900/20 p-4 rounded-2xl">
                <p class="text-xs text-blue-600 dark:text-blue-400 font-bold uppercase mb-1">Solde Retirable (Livré)</p>
                <p class="text-2xl font-bold text-blue-700 dark:text-blue-300">{{ formatPrice(availableBalance) }}</p>
            </div>
            ...
            <form @submit.prevent="handleWithdraw" class="space-y-4">
                <div>
                    ...
                    <input 
                        ...
                        class="w-full bg-gray-50 dark:bg-gray-700 border-none rounded-2xl p-4 text-lg font-bold focus:ring-2 focus:ring-blue-500"
                        placeholder="0.00"
                    />
                </div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <button 
                            ...
                            :class="[
                                ...
                                withdrawForm.method === 'MonCash' ? 'border-blue-600 bg-blue-50 text-blue-600' : 'border-gray-100 dark:border-gray-700 text-gray-400'
                            ]"
                        >
                        ...
                        <button 
                            type="button"
                            @click="withdrawForm.method = 'Virement'"
                            :class="[
                                'p-4 rounded-2xl border-2 transition-all flex flex-col items-center gap-2',
                                withdrawForm.method === 'Virement' ? 'border-blue-600 bg-blue-50 text-blue-600' : 'border-gray-100 dark:border-gray-700 text-gray-400'
                            ]"
                        >
                            <i class="fas fa-university text-xl"></i>
                            <span class="text-xs font-bold">Virement</span>
                        </button>
                    </div>
                </div>
